...
def setup_database():
    ...
    try:
        # 1. 创建 std_iso_detail
        print("正在检查/创建 std_iso_detail 表...")
        create_table_sql = """
        CREATE TABLE IF NOT EXISTS `std_iso_detail` (
            `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
            `base_id` BIGINT UNSIGNED NOT NULL COMMENT '关联 std_base.id',
            `std_varsion` TINYINT DEFAULT NULL COMMENT '版本',
            `std_EN` VARCHAR(8) DEFAULT NULL COMMENT '标准语言',
            `std_rele_issue` VARCHAR(50) DEFAULT NULL COMMENT '标准发布组织',
            UNIQUE INDEX `uk_base_id` (`base_id`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ISO国际标准详情表';
        """
        ...
        SELECT b.id, b.std_id, b.std_type, b.std_type_no, b.std_chinesename, b.std_englishname, b.release_date, b.implement_date, b.ex_state, b.create_time,
               ...
               i.std_varsion, i.std_EN, i.std_rele_issue
        FROM std_base b
...
def import_iso_data():
    excel_path = r'E:\Downloads\ISO国际标准目录.xlsx'
    ...
    try:
        for index, row in df.iterrows():
            ...
            # 1. 插入 std_base (使用 INSERT IGNORE)
            base_sql = """
            ...
            # 2. 插入 std_iso_detail
            detail_sql = """
            INSERT INTO std_iso_detail 
            (base_id, std_varsion, std_EN, std_rele_issue) 
            VALUES (%s, %s, %s, %s)
            """
            ...
            try:
                ...
                if pd.notna(std_varsion_val):
                    std_varsion = int(float(std_varsion_val))
                else:
                    std_varsion = None
            except:
                std_varsion = None
